...
export default function TransactionsDialog() {
  ...
  useEffect(() => {
    async function fetchTransactions() {
      if (!isOpen) return; // fetch only when dialog is open
      setLoading(true);
      try {
        const res = await fetch(
          `/api/transactions?page=${page}&status=${filter}&search=${search}`
        );
  ...
  return (
    <>
      {/* Trigger Button */}
      <Button onClick={() => setIsOpen(true)}>View Transactions</Button>
      ...
      >
        <Column gap="lg" p="md" fillWidth style={{ maxHeight: "80vh", overflowY: "auto" }}>
          <Row gap="md" wrap>
            <Input
